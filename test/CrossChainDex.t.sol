...
contract CrossChainDexTest is Test {
    ...
    IRouterClient private sourceRouter;
    IRouterClient private destinationRouter;
    ...
    function setUp() public {
        ...
        // avaxFujiFork = vm.createFork(AVALANCHE_FUJI_RPC_URL);
        ...
        sourceRouter = IRouterClient(FUJI_ROUTER_ADDRESS);

        // Step 1) Deploy CrossChainDex.sol to Avalanche Fuji
        assertEq(vm.activeFork(), avaxFujiFork);
        senderCrossChainDex = new CrossChainDex(address(sourceRouter), FUJI_CHAIN_SELECTOR);
        console.log("CrossChainDex deployed to: ", address(senderCrossChainDex));
        vm.makePersistent(address(ccipLocalSimulatorFork), address(sourceRouter), address(senderCrossChainDex));
        ...
        destinationRouter = IRouterClient(SEPOLIA_ROUTER_ADDRESS);
        receiverCrossChainDex = new CrossChainDex(address(destinationRouter), SEPOLIA_CHAIN_SELECTOR);
